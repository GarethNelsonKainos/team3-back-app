	...
	it("GET /api/applications/cv - redirects admin to signed CV URL", async () => {
		...
		const fakeFileStorageClient = {
			getDownloadUrl: vi
				.fn()
				.mockResolvedValue(
					"https://signed.example.com/applications/test-cv.pdf",
				),
		} as any;
		...
		app.get(
			...
			async (req, res) => {
				...
				try {
					...
					const downloadUrl =
						await fakeFileStorageClient.getDownloadUrl(normalizedKey);
		...
		const token = jwt.sign(
			{ sub: 99, email: "admin@example.com", role: UserRole.ADMIN },
		...
		expect(fakeFileStorageClient.getDownloadUrl).toHaveBeenCalledWith(
