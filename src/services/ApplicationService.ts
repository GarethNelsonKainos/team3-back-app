...
export class ApplicationService {
  ...
  constructor(applicationDao: ApplicationDao, fileStorageClient: FileStorageClient | null = null) {
  ...
  async hireApplicant(applicationId: number): Promise<ApplicationResponse> {
    ...
    if (application.applicationStatus !== ApplicationStatus.InProgress) {
      throw new InvalidApplicationStatusError(`Cannot hire: application status is "${application.applicationStatus}"`);
    }
    if (application.jobRole.numberOfOpenPositions <= 0) {
      throw new NoOpenPositionsError("Cannot hire: no open positions available for this role");
    }
    const updated = await this.applicationDao.updateApplicationStatus(applicationId, ApplicationStatus.Hired);
  ...
  async rejectApplicant(applicationId: number): Promise<ApplicationResponse> {
    ...
    if (application.applicationStatus !== ApplicationStatus.InProgress) {
      throw new InvalidApplicationStatusError(`Cannot reject: application status is "${application.applicationStatus}"`);
    }
    const updated = await this.applicationDao.updateApplicationStatus(applicationId, ApplicationStatus.Rejected);
    return {
      applicationId: updated.applicationId,
      userId: updated.user.userId,
      email: updated.user.email,
